...
/*   Updated: 2017/12/14 17:24:12 by bait-sli         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "lem_in.h"

static void check_cmd(char *line, t_env *env)
{
    if(ft_is_cmd(line) == 1 && env->verif.start_ok == 0 && env->verif.end_ok != 1)
      env->verif.start_ok = 1;
    if(ft_is_cmd(line) == 2 && env->verif.end_ok == 0 && env->verif.start_ok != 1)
      env->verif.end_ok = 1;
}
...
static int check_line(char *line, t_env *env)
{
  ...
  {
    ft_add_vertice(&env->map, line, env);
    return(1);
  }
  if(ft_is_cmd(line) != 0)
  {
  ...
  if(ft_is_pipe(line))
  {
    ...
    ft_add_pipe(&env->network, line, env);
    return(1);
  }
  return(0);
}
